...
var Embed=(function(){  'use strict';
  ...
  function publicMethod() {console.log(publicProperty); _privateMethod();} // I can call `privateMethod()` you know...
  ///                                                        
  function _Node(_idnode){return document.getElementById(_idnode);}
  function _parentNode(_idnode){ //query parentNode id if none get it body
    var x = _Node(_idnode);
    if (x) {return x;} //parent there x
    else {return document.body;};
  }

  function _includeHTML() { /* embed HTML elements from a page XMLHttpRequest*/
  ...
  function _new_HTML(_idPARENT, Attribute_html) {/* Plugin HTML elements into created embed-div. */
    var NewDIV = document.createElement('div'); 
    NewDIV.textContent = "(NewDIVtoEmbedHTML.textContent)";
  ...
  function _new_DIV(_idPARENT,_idnode) {var NewDIV = document.createElement('div');							//create _new_DIV
    NewDIV.id = _idnode; NewDIV.textContent =  "This"+"@div("+_idnode+')@'+_idPARENT;
    _parentNode(_idPARENT).appendChild(NewDIV);
  }
  function _new_PARAGRAPH(_idPARENT,_idnode,_newTEXT) {var para = document.createElement("P");	//create _new_PARAGRAPH    
    ...
    para.appendChild(t); para.id = _idnode;
    _parentNode(_idPARENT).appendChild(para);
  }
  function _new_SPAN(_idPARENT,_idnode) {var _span = document.createElement('span');						//create _new_SPAN 
    _span.id = _idnode; _span.textContent = "This"+"@span("+_idnode+')@'+_idPARENT;
    _parentNode(_idPARENT).appendChild(_span)
  }
  function _new_UL(_idPARENT,_idnode) {var _UL = document.createElement('UL');          				//create _new_UL    
    _UL.id = _idnode;
		//_UL.setAttribute("aria-label", _idnode+"@ul("+_idnode+')@'+_idPARENT);
		//_UL.setAttribute("style",'#'+_idnode+':before{content:attr(aria-label);font-size:120%;font-weight:bold;margin-left:-15px;}');
		_UL.textContent = "This"+"@ul("+_idnode+')@'+_idPARENT;
  ...
  function _new_BUTTON(_idPARENT,_idnode) {var btn = document.createElement("BUTTON");					//create _new_BUTTON
    ...
    btn.appendChild(t); btn.setAttribute("id", _idnode);
    _parentNode(_idPARENT).appendChild(btn);
  }
  ...
  function _new_MEDIA(_idPARENT,_idnode) {var swf = document.createElement("EMBED");						//create _new_MEDIA
	...
	function _new_SCRIPT(_idPARENT,_idnode,CODE){var script = document.createElement("script");		//create _new_SCRIPT
    script.id = _idnode;
    _parentNode(_idPARENT).appendChild(script);
		document.getElementById(_idnode).textContent = CODE;
	}

  function _Resize(_idnode,_height,_width) {var x = document.getElementById(_idnode); x.height= _height; x.width = _width;}
  function _Attribute(_idnode,_idAttribute,_Specify) {document.getElementById(_idnode).setAttribute(_idAttribute, _Specify);}
  function _Remove(_idnode) {document.getElementById(_idnode).remove();}

  return {/// declare public variables and/or functions
    HTML:  	_new_HTML,
    SCRIPT: _new_SCRIPT,
    SCRIPTSRC: _new_SCRIPT_src,
    DIV:    _new_DIV,
    PARA:   _new_PARAGRAPH,
    SPAN:   _new_SPAN,
    UL:     _new_UL,
...
function method() {
  ...
  Embed.SCRIPT(_idPARENT,_idnode,CODE);
  Embed.SCRIPTSRC(_Specify);
  ...
  Embed.ATTR(_idnode,_idAttribute,_Specify);
	Embed.NODE(_idnode);
